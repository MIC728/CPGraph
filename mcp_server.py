...
【一级关系查询】（<1秒）- 适用于：直接相关概念、简单查询
- 向量搜索 + 直接邻居
- 精确匹配 + 直接关系

【二级关系查询】（~3-4秒）- 适用于：相关扩展、题目查找（推荐）
- 向量搜索 + 二级关系（示例6）
- 精确ID + 二级关系

【多源邻域交集】（~5-6秒）- 适用于：综合题目、跨领域分析
- 多源向量搜索 + 邻域交集（示例7）

🎯 场景推荐：
- 查找相似题目：使用"向量搜索 + 二级关系"（示例6）
- 综合分析：使用"多源向量搜索 + 邻域交集"（示例7）
- 已知实体扩展：使用"精确ID + 多级关系"（示例5）

💡 性能提示：5-6秒的查询时间是可接受的，能够显著提升查询效果！
...
【查询策略选择】
1. 简单查询（<1秒）：直接使用一级关系查询（示例1-2）
   - 适用：查找直接相关概念、精确匹配已知实体

2. 深度查询（3-6秒）：使用二级关系查询（示例6-7）
   - 适用：查找间接相关题目、对比分析、深度探索
   - 性能提示：5-6秒的查询时间是可接受的，能够显著提升查询效果！

3. 复杂场景（5-6秒）：使用多源查询 + 多级关系 + 交集（示例7）
   - 适用：全面分析、复杂场景
...
- 结合嵌入向量相似topk和二级邻域的高级查询手段值得使用（5-6s）
- LLM应该根据查询复杂度按需选择查询手段，而不是每次都保守使用开销小的查询
"""
)
async def execute_custom_cypher(
    query: str,
