...
class NetworkXStorage(BaseGraphStorage):
    ...
    async def get_knowledge_graph(
        ...
        for node in subgraph.nodes():
            ...
            # Get entity_type_dim1 and entity_type_dim2 as labels
            labels = []
            if "entity_type_dim1" in node_data:
                if isinstance(node_data["entity_type_dim1"], list):
                    labels.extend(node_data["entity_type_dim1"])
                else:
                    labels.append(node_data["entity_type_dim1"])
            if "entity_type_dim2" in node_data:
                if isinstance(node_data["entity_type_dim2"], list):
                    labels.extend(node_data["entity_type_dim2"])
                else:
                    labels.append(node_data["entity_type_dim2"])
